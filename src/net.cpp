...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Bitcoindiamond " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"bitcoindiamondtools.com", "89.74.31.47"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bitcoindiamond is probably already running."), addrBind.ToString().c_str());
        else
