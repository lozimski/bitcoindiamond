// Copyright (c) 2017-2018 Sabrina Nakamoto
...
#if defined(NDEBUG)
# error "Bitcoindiamond cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe2af7f3aa72b99b821fb6924f79415d5485846fb157da51edd01fc001f5f132e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoindiamond: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoindiamond Signed Message:\n";
...
{
    // Bitcoindiamond: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 315360 blocks, which will occur approximately every 3 years
    nSubsidy >>= (nHeight / 315360); // Bitcoindiamond: 315.36k blocks in ~ 3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Bitcoindiamond: 7 days
static const int64 nTargetSpacing = 5 * 60; // Bitcoindiamond: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoindiamond: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoindiamond: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe2af7f3aa72b99b821fb6924f79415d5485846fb157da51edd01fc001f5f132e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin Diamond Birthday 3/NOV/2017 LONDON";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 16 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("212734710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1509698407;
        ...
        assert(block.hashMerkleRoot == uint256("0x10893b0230667d78d75e5b6132dc7f503ec408fc6d2b1a9cf407013abd01ce06"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bitcoindiamond: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // Limit to betweeen 3K and MAX_BLOCK_SIZE-3K for sanity:
    nBlockMaxSize = std::max((unsigned int)3000, std::min((unsigned int)(MAX_BLOCK_SIZE-3000), nBlockMaxSize));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoindiamondMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoindiamondMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoindiamondMiner(CWallet *pwallet)
{
    printf("BitcoindiamondMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoindiamond-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoindiamondMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoindiamondMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoindiamondMiner, pwallet));
}
